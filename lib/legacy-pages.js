...
function stripNavAndFooter(html) {
  return html
    ...
    .replace(/<div\s+class="mobile-menu"[\s\S]*?<\/div>\s*(?=\n)/gi, '')
...
const loadLegacyPage = cache((relativePath) => {
  ...
  return {
    ...
    bodyHtml: rewriteInternalLinks(stripNavAndFooter(stripScripts(extractBodyContent(html)))),
  };
});
...
export function buildMetadata(page, routePath, overrides = {}) {
  if (!page) {
    return {};
  }
  ...
  const metadata = {
    ...
    description,
    openGraph: {
      title: overrides.title || page.ogTitle || page.title,
      description: overrides.description || page.ogDescription || page.description,
      type: 'website',
    },
    twitter: {
